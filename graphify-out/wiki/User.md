...
> God node · 115 connections · [E:\Non_Office\Dev_Space\vibe_skool\veltrics\src\backend\app\models\user.py](file:///E:/Non_Office/Dev_Space/vibe_skool/veltrics/src/backend/app/models/user.py#L13)
...
sequenceDiagram
    ...
    participant P199 as AuditLog
    participant P200 as AuthSessionDTO
    participant P201 as SessionRevokeResponse
    participant P202 as UC-004: User Password Authentication & Session Initiation     Authenticates reg
    participant P203 as UC-006: Forgot Password Request     Issues a password reset JWT token with 5-mi
    participant P204 as UC-006: Password Reset Execution     Verifies reset token, validates password p
    participant P205 as UC-009: Session Refresh & Access Token Renewal     Validates active refresh tok
    participant P206 as UC-010: User Sign Out & Token Revocation     Revokes the provided refresh token
    participant P207 as UserDTO
    participant P208 as OrganizationDTO
    participant P209 as UserSessionDTO
    participant P210 as AuditService
    participant P211 as ForgotPasswordResponse
    participant P212 as ResetPasswordResponse
    participant P213 as RefreshTokenResponse
    participant P214 as LogoutResponse
    participant P215 as UserOrganization
    participant P216 as UserSession
    participant P217 as RegisterRequest
    participant P218 as LoginRequest
    participant P219 as ForgotPasswordRequest
    participant P220 as ResetPasswordRequest
    participant P221 as RefreshTokenRequest
    participant P222 as LogoutRequest
    participant P223 as AC 1: GIVEN an authenticated user session with a valid refresh token     WHEN s
    participant P224 as AC 2: GIVEN a revoked refresh token     WHEN submitted to POST /api/v1/auth/ref
    participant P225 as AC 3: GIVEN a malformed or invalid refresh token     WHEN submitted to POST /ap
    participant P226 as AC 4: GIVEN a refresh token that has already been revoked     WHEN logout is ca
    participant P227 as AC 1: WHEN GET /api/v1/users/me/sessions is called THE SYSTEM SHALL return activ
    participant P228 as AC 2: User can revoke a specific session.
    participant P229 as Acceptance Criterion: WHEN a session is revoked via API THE SYSTEM SHALL block a
    participant P230 as A1: User can revoke all other active sessions.
    participant P231 as AC 1: WHEN a registered user requests a password reset link/token for their emai
    participant P232 as AC 2: WHEN a reset request is submitted for a non-existent email     THE SYSTEM
    participant P233 as AC 3: WHEN a user submits a valid reset token and strong new password     THE S
    participant P234 as AC 4: WHEN an invalid token or token of wrong type (e.g. access token) is submit
    participant P235 as AC 5: WHEN a new password does not satisfy password policy (min 8 chars, 1 upper
    participant P236 as AC 1: GIVEN an authenticated registered user     WHEN they request GET /api/v1/
    participant P237 as AC 2: GIVEN an authenticated user updating their profile details     WHEN they
    participant P238 as AC 3: GIVEN a profile update request with invalid full name length (< 2 characte
    participant P239 as AC 4: GIVEN an unauthenticated request to profile endpoints without user identit
    participant P240 as AC 1: GIVEN a valid active refresh token     WHEN submitted to POST /api/v1/aut
    participant P241 as AC 2: GIVEN an invalid, expired, or non-refresh token (e.g. access or reset toke
    participant P242 as AC 3: GIVEN a refresh token for a disabled or soft-deleted user account     WHE
    participant P243 as AC 4: GIVEN a newly issued access token from POST /api/v1/auth/refresh     WHEN
    participant P244 as test_setup()
    P0->>+ P1: uses
    P1-->>- P0: return
    P1->>+ P2: uses
    P2-->>- P1: return
