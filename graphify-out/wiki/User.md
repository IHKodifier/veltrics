...
> God node · 131 connections · [E:\Non_Office\Dev_Space\vibe_skool\veltrics\src\backend\app\models\user.py](file:///E:/Non_Office/Dev_Space/vibe_skool/veltrics/src/backend/app/models/user.py#L13)
...
sequenceDiagram
    ...
    participant P239 as AC 1: GIVEN an authenticated user session with a valid refresh token     WHEN s
    participant P240 as AC 2: GIVEN a revoked refresh token     WHEN submitted to POST /api/v1/auth/ref
    participant P241 as AC 3: GIVEN a malformed or invalid refresh token     WHEN submitted to POST /ap
    participant P242 as AC 4: GIVEN a refresh token that has already been revoked     WHEN logout is ca
    participant P243 as AC 1: WHEN GET /api/v1/users/me/sessions is called THE SYSTEM SHALL return activ
    participant P244 as AC 2: User can revoke a specific session.
    participant P245 as Acceptance Criterion: WHEN a session is revoked via API THE SYSTEM SHALL block a
    participant P246 as A1: User can revoke all other active sessions.
    participant P247 as AC 1: WHEN a registered user requests a password reset link/token for their emai
    participant P248 as AC 2: WHEN a reset request is submitted for a non-existent email     THE SYSTEM
    participant P249 as AC 3: WHEN a user submits a valid reset token and strong new password     THE S
    participant P250 as AC 4: WHEN an invalid token or token of wrong type (e.g. access token) is submit
    participant P251 as AC 5: WHEN a new password does not satisfy password policy (min 8 chars, 1 upper
    participant P252 as AC 1: GIVEN an authenticated registered user     WHEN they request GET /api/v1/
    participant P253 as AC 2: GIVEN an authenticated user updating their profile details     WHEN they
    participant P254 as AC 3: GIVEN a profile update request with invalid full name length (< 2 characte
    participant P255 as AC 4: GIVEN an unauthenticated request to profile endpoints without user identit
    participant P256 as AC 1: GIVEN a valid active refresh token     WHEN submitted to POST /api/v1/aut
    participant P257 as AC 2: GIVEN an invalid, expired, or non-refresh token (e.g. access or reset toke
    participant P258 as AC 3: GIVEN a refresh token for a disabled or soft-deleted user account     WHE
    participant P259 as AC 4: GIVEN a newly issued access token from POST /api/v1/auth/refresh     WHEN
    participant P260 as test_setup()
    P0->>+ P1: uses
    P1-->>- P0: return
    P1->>+ P2: uses
    P2-->>- P1: return
