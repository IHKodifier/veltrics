...
> God node · 65 connections · [E:\Non_Office\Dev_Space\vibe_skool\veltrics\src\backend\app\models\maintenance.py](file:///E:/Non_Office/Dev_Space/vibe_skool/veltrics/src/backend/app/models/maintenance.py#L7)
...
sequenceDiagram
    ...
    participant P216 as Vehicle
    participant P217 as AuditLog
    participant P218 as ServiceRecord
    participant P219 as OrganizationInvitation
    participant P220 as Driver
    participant P221 as UserOrganization
    participant P222 as FuelLog
    participant P223 as ExpenseLog
    participant P224 as OrganizationInvitationResponse
    participant P225 as SwitchOrganizationResponse
    participant P226 as OrganizationCreate
    participant P227 as PersonalOrganizationCreate
    participant P228 as OrganizationResponse
    participant P229 as SwitchOrganizationRequest
    participant P230 as OrganizationInvitationCreate
    participant P231 as Trip
    participant P232 as OrganizationUpdate
    participant P233 as UC-035: Create custom maintenance schedule task item for a vehicle.
    participant P234 as UC-035: Update schedule parameters (intervals, task name, active status) and rec
    participant P235 as UC-035: Soft-delete schedule item.
    participant P236 as UC-037: Retrieve chronological service records for a vehicle within the active o
    participant P237 as UC-038: Bulk accept/acknowledge default maintenance schedules for a vehicle.
    participant P238 as UC-064: Get high-level KPI dashboard metrics summary for active organization.
    participant P239 as UC-065: Cost Breakdown Charts per Vehicle (Fuel vs Maintenance vs Expenses).
    participant P240 as create_maintenance_schedule()
    participant P241 as bulk_accept_maintenance_schedules()
    participant P242 as get_maintenance_schedules()
    P0->>+ P1: uses
    P1-->>- P0: return
    P1->>+ P2: uses
    P2-->>- P1: return
