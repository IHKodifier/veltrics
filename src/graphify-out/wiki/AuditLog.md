...
> God node · 32 connections · [E:\Non_Office\Dev_Space\vibe_skool\veltrics\src\backend\app\models\audit_log.py](file:///E:/Non_Office/Dev_Space/vibe_skool/veltrics/src/backend/app/models/audit_log.py#L5)
...
sequenceDiagram
    ...
    participant P223 as log_event()
    participant P224 as AC 1: GIVEN an authenticated user session with a valid refresh token     WHEN s
    participant P225 as AC 2: GIVEN a revoked refresh token     WHEN submitted to POST /api/v1/auth/ref
    participant P226 as AC 3: GIVEN a malformed or invalid refresh token     WHEN submitted to POST /ap
    participant P227 as AC 4: GIVEN a refresh token that has already been revoked     WHEN logout is ca
    participant P228 as update_vehicle()
    participant P229 as UC-012: Audit Log Recording for Authentication Events.         Extracts event m
    P0->>+ P1: uses
    P1-->>- P0: return
    P1->>+ P2: uses
    P2-->>- P1: return
...
- [[AC 1: GIVEN an authenticated user     WHEN DELETE /api/v1/users/me is invoked]] `INFERRED`
- [[AC 2: GIVEN a user who is the sole owner of an active non-personal organization]] `INFERRED`
- [[AC 3: GIVEN a soft-deleted user account     WHEN attempting to authenticate, ac]] `INFERRED`
- [[AC 1: System creates immutable AuditLog entry upon user registration.]] `INFERRED`
- [[AC 2: System records USER_LOGIN_SUCCESS audit entry with IP & User-Agent metadat]] `INFERRED`
- [[A1: Unauthenticated attempt records USER_LOGIN_FAILURE with actor_id = None.]] `INFERRED`
- [[AC 3: System records USER_PASSWORD_RESET_REQUEST and USER_PASSWORD_RESET_SUCCESS]] `INFERRED`
- [[AC 4: System records USER_LOGOUT audit entry upon session termination.]] `INFERRED`
- [[Edge Case: Audit log DB write exception is handled safely and does not block use]] `INFERRED`
- [[Test 1: Verify PATCH /api/v1/vehicles/{vehicle_id} successfully updates vehicle]] `INFERRED`
- [[Test 2: Verify updating vehicle under wrong organization_id returns HTTP 403 For]] `INFERRED`
