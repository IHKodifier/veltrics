...
sequenceDiagram
    ...
    participant P272 as log_event()
    participant P273 as execute_sync_batch()
    participant P274 as accept_organization_invitation()
    participant P275 as redeem_organization_invitation_code()
    participant P276 as reconcile_safepay_webhook()
    participant P277 as create_organization_invitation()
    participant P278 as submit_enterprise_inquiry()
    participant P279 as update_organization_profile()
    participant P280 as remove_organization_member()
    participant P281 as cancel_organization_invitation()
    participant P282 as verify_and_claim_ad_reward()
    participant P283 as AC 1: GIVEN an authenticated user session with a valid refresh token     WHEN s
    participant P284 as AC 2: GIVEN a revoked refresh token     WHEN submitted to POST /api/v1/auth/ref
    participant P285 as AC 3: GIVEN a malformed or invalid refresh token     WHEN submitted to POST /ap
    participant P286 as AC 4: GIVEN a refresh token that has already been revoked     WHEN logout is ca
    participant P287 as soft_delete_organization()
    participant P288 as update_vehicle()
    participant P289 as delete_vehicle()
    participant P290 as update_vehicle_odometer()
    participant P291 as restore_deleted_vehicle()
    participant P292 as assign_primary_driver()
    participant P293 as unassign_primary_driver()
    participant P294 as cancel_subscription()
    participant P295 as execute_downgrade_protocol()
    participant P296 as UC-012: Audit Log Recording for Authentication Events.         Extracts event m
    P0->>+ P1: uses
    P1-->>- P0: return
    P1->>+ P2: uses
    P2-->>- P1: return
