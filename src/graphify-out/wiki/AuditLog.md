...
> God node · 76 connections · [E:\Non_Office\Dev_Space\vibe_skool\veltrics\src\backend\app\models\audit_log.py](file:///E:/Non_Office/Dev_Space/vibe_skool/veltrics/src/backend/app/models/audit_log.py#L5)
...
sequenceDiagram
    ...
    participant P257 as log_event()
    participant P258 as execute_sync_batch()
    participant P259 as accept_organization_invitation()
    participant P260 as redeem_organization_invitation_code()
    participant P261 as create_organization_invitation()
    participant P262 as update_organization_profile()
    participant P263 as remove_organization_member()
    participant P264 as cancel_organization_invitation()
    participant P265 as AC 1: GIVEN an authenticated user session with a valid refresh token     WHEN s
    participant P266 as AC 2: GIVEN a revoked refresh token     WHEN submitted to POST /api/v1/auth/ref
    participant P267 as AC 3: GIVEN a malformed or invalid refresh token     WHEN submitted to POST /ap
    participant P268 as AC 4: GIVEN a refresh token that has already been revoked     WHEN logout is ca
    participant P269 as soft_delete_organization()
    participant P270 as update_vehicle()
    participant P271 as delete_vehicle()
    participant P272 as update_vehicle_odometer()
    participant P273 as restore_deleted_vehicle()
    participant P274 as assign_primary_driver()
    participant P275 as unassign_primary_driver()
    participant P276 as UC-012: Audit Log Recording for Authentication Events.         Extracts event m
    P0->>+ P1: uses
    P1-->>- P0: return
    P1->>+ P2: uses
    P2-->>- P1: return
