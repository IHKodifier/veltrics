...
> God node · 136 connections · [E:\Non_Office\Dev_Space\vibe_skool\veltrics\src\backend\app\models\vehicle.py](file:///E:/Non_Office/Dev_Space/vibe_skool/veltrics/src/backend/app/models/vehicle.py#L20)
...
sequenceDiagram
    ...
    participant P234 as MaintenanceSchedule
    participant P235 as AuditLog
    participant P236 as ServiceRecord
    participant P237 as Driver
    participant P238 as UserOrganization
    participant P239 as OrganizationInvitation
    ...
    participant P251 as UC-035: Create custom maintenance schedule task item for a vehicle.
    participant P252 as UC-035: Update schedule parameters (intervals, task name, active status) and rec
    participant P253 as UC-035: Soft-delete schedule item.
    participant P254 as UC-037: Retrieve chronological service records for a vehicle within the active o
    participant P255 as UC-038: Bulk accept/acknowledge default maintenance schedules for a vehicle.
    participant P256 as UC-064: Get high-level KPI dashboard metrics summary for active organization.
    participant P257 as UC-065: Cost Breakdown Charts per Vehicle (Fuel vs Maintenance vs Expenses).
    participant P258 as create_vehicle()
    participant P259 as test_uc096_delta_sync_with_since_timestamp()
    participant P260 as test_uc094_sync_conflict_server_wins()
    participant P261 as test_uc096_delta_sync_full_snapshot()
    participant P262 as test_uc031_restore_vehicle_quota_exceeded()
    P0->>+ P1: uses
    P1-->>- P0: return
    P1->>+ P2: uses
    P2-->>- P1: return
