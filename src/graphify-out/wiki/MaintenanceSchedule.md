...
> God node · 103 connections · [E:\Non_Office\Dev_Space\vibe_skool\veltrics\src\backend\app\models\maintenance.py](file:///E:/Non_Office/Dev_Space/vibe_skool/veltrics/src/backend/app/models/maintenance.py#L7)

## Call Trace Diagram

```mermaid
sequenceDiagram
    participant P0 as MaintenanceSchedule
    ...
    participant P275 as Vehicle
    participant P276 as ServiceRecord
    participant P277 as AuditLog
    participant P278 as UserOrganization
    participant P279 as Driver
    participant P280 as FuelLog
    participant P281 as OrganizationInvitation
    participant P282 as ExpenseLog
    participant P283 as Trip
    participant P284 as OrganizationInvitationResponse
    participant P285 as SwitchOrganizationResponse
    participant P286 as OrganizationCreate
    participant P287 as PersonalOrganizationCreate
    participant P288 as OrganizationResponse
    participant P289 as SwitchOrganizationRequest
    participant P290 as OrganizationInvitationCreate
    participant P291 as OrganizationUpdate
    participant P292 as UC-064: Get high-level KPI dashboard metrics summary for active organization.
    participant P293 as UC-067: Fleet Manager Web Dashboard Layout & Aggregation.
    participant P294 as UC-068: Fleet Cost Ranking Table & Heatmap.
    participant P295 as UC-069: Fleet Vehicle Availability Widget.
    participant P296 as UC-071: Get customized dashboard layout preferences for current user.
    participant P297 as UC-071: Save customized dashboard layout preferences for current user.
    participant P298 as UC-035: Create custom maintenance schedule task item for a vehicle.
    participant P299 as UC-035: Update schedule parameters (intervals, task name, active status) and rec
    participant P300 as UC-035: Soft-delete schedule item.
    participant P301 as UC-039: Add Custom Maintenance Service Item.
    participant P302 as UC-040: Edit existing service record.
    participant P303 as UC-041: Delete service record (soft delete).
    participant P304 as UC-037 & UC-042: Retrieve & filter service records.
    participant P305 as UC-045: Snooze / Defer Maintenance Alert.
    participant P306 as UC-038: Bulk accept/acknowledge default maintenance schedules for a vehicle.
    participant P307 as UC-037: Retrieve chronological service records for a vehicle within the active o
    participant P308 as UC-038: Bulk accept/acknowledge default maintenance schedules for a vehicle.
    participant P309 as UC-064: Get high-level KPI dashboard metrics summary for active organization.
    participant P310 as UC-065: Cost Breakdown Charts per Vehicle (Fuel vs Maintenance vs Expenses).
    participant P311 as create_maintenance_schedule()
    participant P312 as bulk_accept_maintenance_schedules()
    participant P313 as get_maintenance_schedules()
    participant P314 as add_custom_maintenance_schedule()
    P0->>+ P1: uses
    P1-->>- P0: return
    P1->>+ P2: uses
    P2-->>- P1: return
...
- [[test_data()]] `INFERRED`
- [[create_maintenance_schedule()]] `INFERRED`
- [[bulk_accept_maintenance_schedules()]] `INFERRED`
- [[test_setup()]] `INFERRED`
- [[test_setup()]] `INFERRED`
- [[get_maintenance_schedules()]] `INFERRED`
- [[add_custom_maintenance_schedule()]] `INFERRED`
