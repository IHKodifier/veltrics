...
> God node · 149 connections · [E:\Non_Office\Dev_Space\vibe_skool\veltrics\src\backend\app\models\user.py](file:///E:/Non_Office/Dev_Space/vibe_skool/veltrics/src/backend/app/models/user.py#L13)
...
sequenceDiagram
    ...
    participant P233 as AuditLog
    participant P234 as UserOrganization
    participant P235 as AuthSessionDTO
    participant P236 as SessionRevokeResponse
    participant P237 as UC-004: User Password Authentication & Session Initiation     Authenticates reg
    participant P238 as UC-006: Forgot Password Request     Issues a password reset JWT token with 5-mi
    participant P239 as UC-006: Password Reset Execution     Verifies reset token, validates password p
    participant P240 as UC-009: Session Refresh & Access Token Renewal     Validates active refresh tok
    participant P241 as UC-010: User Sign Out & Token Revocation     Revokes the provided refresh token
    participant P242 as UserDTO
    participant P243 as OrganizationDTO
    participant P244 as UserSessionDTO
    participant P245 as AuditService
    participant P246 as ForgotPasswordResponse
    participant P247 as ResetPasswordResponse
    participant P248 as RefreshTokenResponse
    participant P249 as LogoutResponse
    participant P250 as UserSession
    participant P251 as RegisterRequest
    participant P252 as LoginRequest
    participant P253 as ForgotPasswordRequest
    participant P254 as ResetPasswordRequest
    participant P255 as RefreshTokenRequest
    participant P256 as LogoutRequest
    participant P257 as AC 1: GIVEN an authenticated user session with a valid refresh token     WHEN s
    participant P258 as AC 2: GIVEN a revoked refresh token     WHEN submitted to POST /api/v1/auth/ref
    participant P259 as AC 3: GIVEN a malformed or invalid refresh token     WHEN submitted to POST /ap
    participant P260 as AC 4: GIVEN a refresh token that has already been revoked     WHEN logout is ca
    participant P261 as AC 1: WHEN GET /api/v1/users/me/sessions is called THE SYSTEM SHALL return activ
    participant P262 as AC 2: User can revoke a specific session.
    participant P263 as Acceptance Criterion: WHEN a session is revoked via API THE SYSTEM SHALL block a
    participant P264 as A1: User can revoke all other active sessions.
    participant P265 as AC 1: WHEN a registered user requests a password reset link/token for their emai
    participant P266 as AC 2: WHEN a reset request is submitted for a non-existent email     THE SYSTEM
    participant P267 as AC 3: WHEN a user submits a valid reset token and strong new password     THE S
    participant P268 as AC 4: WHEN an invalid token or token of wrong type (e.g. access token) is submit
    participant P269 as AC 5: WHEN a new password does not satisfy password policy (min 8 chars, 1 upper
    participant P270 as AC 1: GIVEN an authenticated registered user     WHEN they request GET /api/v1/
    participant P271 as AC 2: GIVEN an authenticated user updating their profile details     WHEN they
    participant P272 as AC 3: GIVEN a profile update request with invalid full name length (< 2 characte
    participant P273 as AC 4: GIVEN an unauthenticated request to profile endpoints without user identit
    participant P274 as AC 1: GIVEN a valid active refresh token     WHEN submitted to POST /api/v1/aut
    participant P275 as AC 2: GIVEN an invalid, expired, or non-refresh token (e.g. access or reset toke
    participant P276 as AC 3: GIVEN a refresh token for a disabled or soft-deleted user account     WHE
    participant P277 as AC 4: GIVEN a newly issued access token from POST /api/v1/auth/refresh     WHEN
    participant P278 as test_setup()
    P0->>+ P1: uses
    P1-->>- P0: return
    P1->>+ P2: uses
    P2-->>- P1: return
