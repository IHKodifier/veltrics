...
> God node · 169 connections · [E:\Non_Office\Dev_Space\vibe_skool\veltrics\src\backend\app\models\user.py](file:///E:/Non_Office/Dev_Space/vibe_skool/veltrics/src/backend/app/models/user.py#L13)
...
sequenceDiagram
    ...
    participant P248 as AuditLog
    participant P249 as UserOrganization
    participant P250 as AuthSessionDTO
    participant P251 as SessionRevokeResponse
    participant P252 as UC-004: User Password Authentication & Session Initiation     Authenticates reg
    participant P253 as UC-006: Forgot Password Request     Issues a password reset JWT token with 5-mi
    participant P254 as UC-006: Password Reset Execution     Verifies reset token, validates password p
    participant P255 as UC-009: Session Refresh & Access Token Renewal     Validates active refresh tok
    participant P256 as UC-010: User Sign Out & Token Revocation     Revokes the provided refresh token
    participant P257 as UserDTO
    participant P258 as OrganizationDTO
    participant P259 as UserSessionDTO
    participant P260 as AuditService
    participant P261 as ForgotPasswordResponse
    participant P262 as ResetPasswordResponse
    participant P263 as RefreshTokenResponse
    participant P264 as LogoutResponse
    participant P265 as UserSession
    participant P266 as RegisterRequest
    participant P267 as LoginRequest
    participant P268 as ForgotPasswordRequest
    participant P269 as ResetPasswordRequest
    participant P270 as RefreshTokenRequest
    participant P271 as LogoutRequest
    participant P272 as UC-081, UC-082 & UC-121: Safepay Webhook Processing & Reconciliation Engine
    participant P273 as UC-083: View Subscription Status & Billing History
    participant P274 as UC-084: Cancel Active Subscription
    participant P275 as UC-085 & UC-120: Process Pro-to-Free Subscription Downgrades & Preserved Quotas
    participant P276 as UC-100 & UC-120 & UC-122: Verify Rewarded Ad Completion Signature Token & Claim
    participant P277 as UC-089: Contact Enterprise Sales Inquiry Form (>25 Fleets).     Submits custom
    participant P278 as test_uc080_checkout_session_safepay()
    participant P279 as AC 1: GIVEN an authenticated user session with a valid refresh token     WHEN s
    participant P280 as AC 2: GIVEN a revoked refresh token     WHEN submitted to POST /api/v1/auth/ref
    participant P281 as AC 3: GIVEN a malformed or invalid refresh token     WHEN submitted to POST /ap
    participant P282 as AC 4: GIVEN a refresh token that has already been revoked     WHEN logout is ca
    participant P283 as AC 1: WHEN GET /api/v1/users/me/sessions is called THE SYSTEM SHALL return activ
    participant P284 as AC 2: User can revoke a specific session.
    participant P285 as Acceptance Criterion: WHEN a session is revoked via API THE SYSTEM SHALL block a
    participant P286 as A1: User can revoke all other active sessions.
    participant P287 as AC 1: WHEN a registered user requests a password reset link/token for their emai
    participant P288 as AC 2: WHEN a reset request is submitted for a non-existent email     THE SYSTEM
    participant P289 as AC 3: WHEN a user submits a valid reset token and strong new password     THE S
    participant P290 as AC 4: WHEN an invalid token or token of wrong type (e.g. access token) is submit
    participant P291 as AC 5: WHEN a new password does not satisfy password policy (min 8 chars, 1 upper
    participant P292 as AC 1: GIVEN an authenticated registered user     WHEN they request GET /api/v1/
    participant P293 as AC 2: GIVEN an authenticated user updating their profile details     WHEN they
    participant P294 as AC 3: GIVEN a profile update request with invalid full name length (< 2 characte
    participant P295 as AC 4: GIVEN an unauthenticated request to profile endpoints without user identit
    participant P296 as AC 1: GIVEN a valid active refresh token     WHEN submitted to POST /api/v1/aut
    participant P297 as AC 2: GIVEN an invalid, expired, or non-refresh token (e.g. access or reset toke
    participant P298 as AC 3: GIVEN a refresh token for a disabled or soft-deleted user account     WHE
    participant P299 as AC 4: GIVEN a newly issued access token from POST /api/v1/auth/refresh     WHEN
    participant P300 as test_setup()
    P0->>+ P1: uses
    P1-->>- P0: return
    P1->>+ P2: uses
    P2-->>- P1: return
