...
> God node · 214 connections · [E:\Non_Office\Dev_Space\vibe_skool\veltrics\src\backend\app\models\user.py](file:///E:/Non_Office/Dev_Space/vibe_skool/veltrics/src/backend/app/models/user.py#L13)
...
sequenceDiagram
    ...
    participant P298 as UC-064: Get high-level KPI dashboard metrics summary for active organization.
    participant P299 as UC-067: Fleet Manager Web Dashboard Layout & Aggregation.
    participant P300 as UC-068: Fleet Cost Ranking Table & Heatmap.
    participant P301 as UC-069: Fleet Vehicle Availability Widget.
    participant P302 as UC-071: Get customized dashboard layout preferences for current user.
    participant P303 as UC-071: Save customized dashboard layout preferences for current user.
    participant P304 as UC-081, UC-082 & UC-121: Safepay Webhook Processing & Reconciliation Engine
    participant P305 as UC-083: View Subscription Status & Billing History
    participant P306 as UC-084: Cancel Active Subscription
    participant P307 as UC-085 & UC-120: Process Pro-to-Free Subscription Downgrades & Preserved Quotas
    participant P308 as UC-100 & UC-120 & UC-122: Verify Rewarded Ad Completion Signature Token & Claim
    participant P309 as UC-089: Contact Enterprise Sales Inquiry Form (>25 Fleets).     Submits custom
    participant P310 as UC-076: Get In-App Notification Inbox messages.
    participant P311 as UC-076: Mark notification as read.
    participant P312 as UC-077: Get Notification Channel Preferences.
    participant P313 as UC-077: Update Notification Channel Preferences.
    participant P314 as UC-078: Billing & Payment Alert Notifications.
    participant P315 as UC-079: Automatically Purge Stale FCM Tokens (>90 days inactive).
    participant P316 as UC-044: Log Vehicle Safety Inspection Checklist.
    participant P317 as UC-044: Fetch Vehicle Safety Inspections.
    participant P318 as UC-043: Create Maintenance Vendor.
    participant P319 as UC-043: List Maintenance Vendors.
    participant P320 as test_uc080_checkout_session_safepay()
    participant P321 as UC-110: Export Maintenance History to PDF.
    participant P322 as UC-111: Export Fuel & Expense Logs to CSV.
    participant P323 as UC-112: Generate & Email Monthly Fleet Summary PDF.
    participant P324 as AC 1: GIVEN an authenticated user session with a valid refresh token     WHEN s
    participant P325 as AC 2: GIVEN a revoked refresh token     WHEN submitted to POST /api/v1/auth/ref
    participant P326 as AC 3: GIVEN a malformed or invalid refresh token     WHEN submitted to POST /ap
    participant P327 as AC 4: GIVEN a refresh token that has already been revoked     WHEN logout is ca
    participant P328 as AC 1: WHEN GET /api/v1/users/me/sessions is called THE SYSTEM SHALL return activ
    participant P329 as AC 2: User can revoke a specific session.
    participant P330 as Acceptance Criterion: WHEN a session is revoked via API THE SYSTEM SHALL block a
    participant P331 as A1: User can revoke all other active sessions.
    participant P332 as AC 1: WHEN a registered user requests a password reset link/token for their emai
    participant P333 as AC 2: WHEN a reset request is submitted for a non-existent email     THE SYSTEM
    participant P334 as AC 3: WHEN a user submits a valid reset token and strong new password     THE S
    participant P335 as AC 4: WHEN an invalid token or token of wrong type (e.g. access token) is submit
    participant P336 as AC 5: WHEN a new password does not satisfy password policy (min 8 chars, 1 upper
    participant P337 as AC 1: GIVEN an authenticated registered user     WHEN they request GET /api/v1/
    participant P338 as AC 2: GIVEN an authenticated user updating their profile details     WHEN they
    participant P339 as AC 3: GIVEN a profile update request with invalid full name length (< 2 characte
    participant P340 as AC 4: GIVEN an unauthenticated request to profile endpoints without user identit
    participant P341 as AC 1: GIVEN a valid active refresh token     WHEN submitted to POST /api/v1/aut
    participant P342 as AC 2: GIVEN an invalid, expired, or non-refresh token (e.g. access or reset toke
    participant P343 as AC 3: GIVEN a refresh token for a disabled or soft-deleted user account     WHE
    participant P344 as AC 4: GIVEN a newly issued access token from POST /api/v1/auth/refresh     WHEN
    participant P345 as test_setup()
    P0->>+ P1: uses
    P1-->>- P0: return
    P1->>+ P2: uses
    P2-->>- P1: return
...
- [[test_setup()]] `INFERRED`
- [[test_uc080_checkout_session_safepay()]] `INFERRED`
- [[setup_db()]] `INFERRED`
- [[setup_db()]] `INFERRED`
- [[test_setup()]] `INFERRED`
- [[test_setup()]] `INFERRED`
